...
user_id = input("აირჩიეთ თქვენთვის სასურველი სტუდენტის ID: ")

student_info = next((student for student in my_dict["students"] if str(student["id"]) == user_id), None)

if student_info:
    print(f"სტუდენტის ინფორმაცია:\nID: {student_info['id']}, Name: {student_info['name']}, Age: {student_info['age']}")
    for subject in my_dict["subjects"]:
        grade = subject["grades"].get(user_id)
        if grade:
            print(f"subject: {subject['name']}, grade: {grade}")
else:
    print("სტუდენტი ამ ID-ით არ მოიძებნა.")     
